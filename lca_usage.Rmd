...
Data tab.  Following are the fields from the file (plus two computed fields) that are available in this application:
...
Following are additional fields in the file which are not currently available in this application:

```
CASE_NUMBER          - Unique identifier assigned to each application.
CASE_SUBMITTED       - Date and time the application was submitted.
DECISION_DATE        - Date of the last significant event or decision.
VISA_CLASS           - valid values are H-1B, E-3 Australian, H-1B1 Chile and H-1B1 Singapore.    
EMPLOYMENT_END_DATE  - Ending date of employment
EMPLOYER_ADDRESS1    - Contact information of the Employer requesting labor certification
EMPLOYER_ADDRESS2    
EMPLOYER_CITY
EMPLOYER_STATE
...
EMPLOYER_PHONE
EMPLOYER_PHONE_EXT
AGENT_ATTORNEY_NAME  - Name of Agent or Attorney filing an H-1B application in employer's behalf.
AGENT_ATTORNEY_CITY  - City of the Agent or Attorney filing an H-1B application in employer's behalf.
AGENT_ATTORNEY_STATE - State of the Agent or Attorney filing an H-1B application in employer's behalf.
SOC_CODE             - Occupational code of job as classified by the Standard Occupational Classification (SOC) System.
NAIC_CODE            - Industry code of employer as classified by the North American Industrial Classification System (NAICS)
FULL_TIME_POSITION   - Y = Full Time Position, N = Part Time Position, or blank 
